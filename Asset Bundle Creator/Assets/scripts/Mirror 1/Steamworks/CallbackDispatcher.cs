...
namespace Steamworks
{
	public static class CallbackDispatcher
	{
		...
		internal static void Register(Callback cb)
		{
			...
			{
				List<Callback> value;
				if (!dictionary.TryGetValue(callbackIdentity, out value))
				{
		...
		internal static void Register(SteamAPICall_t asyncCall, CallResult cr)
		{
			lock (m_sync)
			{
				List<CallResult> value;
				if (!m_registeredCallResults.TryGetValue((ulong)asyncCall, out value))
				{
			...
			while (NativeMethods.SteamAPI_ManualDispatch_GetNextCallback(hSteamPipe, m_pCallbackMsg))
			{
				...
				try
				{
					if (callbackMsg_t.m_iCallback == 703)
					{
						...
						bool pbFailed;
						if (NativeMethods.SteamAPI_ManualDispatch_GetAPICallResult(hSteamPipe, steamAPICallCompleted_t.m_hAsyncCall, intPtr, (int)steamAPICallCompleted_t.m_cubParam, steamAPICallCompleted_t.m_iCallback, out pbFailed))
						{
							lock (m_sync)
							{
								List<CallResult> value;
								if (m_registeredCallResults.TryGetValue((ulong)steamAPICallCompleted_t.m_hAsyncCall, out value))
								{
