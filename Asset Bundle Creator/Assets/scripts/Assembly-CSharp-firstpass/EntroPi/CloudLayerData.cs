...
namespace EntroPi
{
	...
	public class CloudLayerData
	{
		...
		[SerializeField]
		private CloudLayer.BlendMode m_BlendMode;
		...
		public float Coverage
		{
			...
			set
			{
				m_Coverage = Mathf.Clamp01(value);
			}
		}
		...
		public float DirectionAngle
		{
			...
			set
			{
				m_Direction = Mathf.Clamp(value, 0f, 360f);
			}
		}
		...
		public Vector4 TextureTransform
		{
			get
			{
				return new Vector4(m_TextureTiling.x, m_TextureTiling.y, m_TextureOffset.x, m_TextureOffset.y);
			}
		}
		...
		public Vector2 AnimationOffset
		{
			get
			{
				return m_AnimationOffset;
			}
			set
			{
				m_AnimationOffset = value;
			}
		}

		public static Vector2 LimitTextureTiling(Vector2 tiling)
		{
			tiling.x = Mathf.Max(tiling.x, 1f);
			tiling.y = Mathf.Max(tiling.y, 1f);
