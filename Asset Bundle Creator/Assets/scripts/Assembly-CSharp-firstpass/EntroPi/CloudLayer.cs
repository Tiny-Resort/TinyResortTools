...
namespace EntroPi
{
	public class CloudLayer : MonoBehaviour
	{
		...
		public Vector4 TextureTransform
		{
			get
			{
				return new Vector4(m_Tiling.x, m_Tiling.y, m_Offset.x, m_Offset.y);
			}
		}
		...
		public float HorizonAngleFade
		{
			...
			set
			{
				m_HorizonAngleFade = value;
			}
		}

		private void OnValidate()
		{
			m_Tiling.x = Mathf.Max(m_Tiling.x, 1f);
			m_Tiling.y = Mathf.Max(m_Tiling.y, 1f);
		...
		{
			base.enabled &= Debug.Verify(m_Texture != null, "Cloud Layer texture not assigned!");
		}
	}
}
