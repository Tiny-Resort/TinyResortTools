...
public abstract class ES3Reader : IDisposable
{
	...
	public virtual void ReadInto<T>(object obj, ES3Type type)
	{
		if (type == null || type.isUnsupported)
		{
			...
			throw new NotSupportedException("Type of " + (((object)type2 != null) ? type2.ToString() : null) + " is not currently supported, and could not be loaded using reflection.");
		}
		if (type.isCollection)
		{
	...
	internal Type ReadTypeFromHeader<T>()
	{
		...
		if (settings.typeChecking)
		{
			...
			if (type != typeof(T))
			{
				string[] obj = new string[5] { "Trying to load data of type ", null, null, null, null };
				Type typeFromHandle = typeof(T);
				obj[1] = (((object)typeFromHandle != null) ? typeFromHandle.ToString() : null);
				obj[2] = ", but data contained in file is type of ";
