...
public class ES3File
{
	...
	public ES3File()
		: this(new ES3Settings(), true)
	{
	}

	public ES3File(string filePath)
		: this(new ES3Settings(filePath), true)
	{
	}
	...
	public void Sync(ES3Settings settings = null)
	{
		...
		using (ES3Writer eS3Writer = ES3Writer.Create(settings, true, !syncWithFile, false))
		{
	...
	{
		ES3File value;
		if (!cachedFiles.TryGetValue(settings.path, out value))
		{
			value = new ES3File(settings, false);
	...
	{
		ES3File value;
		if (!cachedFiles.TryGetValue(oldSettings.path, out value))
		{
