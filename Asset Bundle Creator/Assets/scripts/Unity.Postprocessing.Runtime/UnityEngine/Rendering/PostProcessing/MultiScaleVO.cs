...
namespace UnityEngine.Rendering.PostProcessing
{
	...
	internal sealed class MultiScaleVO : IAmbientOcclusionMethod
	{
		...
		private void Alloc(CommandBuffer cmd, int id, MipLevel size, RenderTextureFormat format, bool uav)
		{
			...
			{
				width = m_ScaledWidths[(int)size],
				height = m_ScaledHeights[(int)size],
				colorFormat = format,
				depthBufferBits = 0,
				volumeDepth = 1,
		...
		public void GenerateAOMap(CommandBuffer cmd, Camera camera, RenderTargetIdentifier destination, RenderTargetIdentifier? depthMap, bool invert, bool isMSAA)
		{
			...
			{
				int num = 1 << i;
				m_ScaledWidths[i] = (m_ScaledWidths[0] + (num - 1)) / num;
				m_ScaledHeights[i] = (m_ScaledHeights[0] + (num - 1)) / num;
			}
		...
		private void PushRenderCommands(CommandBuffer cmd, int source, int destination, Vector3 sourceSize, float tanHalfFovH, bool isMSAA)
		{
			...
			uint y;
			uint z;
			multiScaleAORender.GetKernelThreadGroupSizes(kernelIndex, out x, out y, out z);
		...
		{
			if (m_AmbientOnlyAO == null || !m_AmbientOnlyAO.IsCreated() || m_AmbientOnlyAO.width != context.width || m_AmbientOnlyAO.height != context.height || m_AmbientOnlyAO.useDynamicScale != context.camera.allowDynamicResolution)
			{
		...
		public void RenderAfterOpaque(PostProcessRenderContext context)
		{
			...
			GenerateAOMap(command, context.camera, m_AmbientOnlyAO, null, false, false);
			PushDebug(context);
