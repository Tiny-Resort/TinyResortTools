...
public class TRPlugin {
    ...
    public bool RequireAPIVersion(string minVersion) {
        ...
        var compareVersion = minVersion.Split('.');
        if (compareVersion.Length < 3) {
            TRTools.LogError($"Version must be in the format: X.X.X");
            return false;
        }

        int.TryParse(compareVersion[0], out var majorCompareVersion);
        int.TryParse(compareVersion[1], out var minorCompareVersion);
        int.TryParse(compareVersion[2], out var patchCompareVersion);

        var versions = LeadPlugin.pluginVersion.Split('.');
        int.TryParse(versions[0], out var majorVersion);
    ...
    public string GetAPIVersion() => LeadPlugin.pluginVersion;
}
