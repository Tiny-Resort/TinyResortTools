...
namespace TinyResort {
    ...
    public static class TRLicences {
        ...
        private static readonly int LicenceTypesCount = Enum.GetNames(typeof(LicenceManager.LicenceTypes)).Length;
        private static readonly List<TRCustomLicence> CustomLicences = new();
        private static readonly Dictionary<string, TRCustomLicence> CustomLicencesDict = new();
        ...
        internal static bool StartPatch(LicenceManager __instance) {
            ...
            for (var i = 1; i < __instance.allLicences.Length; i++) {
                ...
                if (i >= LicenceTypesCount)
                    __instance.allLicences[i] = CustomLicences[i - LicenceTypesCount].info;
                else
            ...
            for (var j = 0; j < licenceButtons.Count; j++) {
        ...
        internal static bool getCurrentMaxLevelPrefix(Licence __instance, ref int __result) {
            ...
            if ((int)__instance.type >= LicenceTypesCount) {
                ...
                for (var i = 1; i <= __instance.maxLevel; i++)
        ...
        internal static void LoadLicences() {
            ...
            for (var i = 0; i < LicenceManager.manage.allLicences.Length; i++)
                if (i < LicenceTypesCount)
                    ...
                    LicenceManager.manage.allLicences[i] = CustomLicences[i - LicenceTypesCount].info;
        }
        ...
        internal static void LoadLicenceData() {
            for (var i = 0; i < CustomLicences.Count; i++) {
                ...
                if (val == null)
                    CustomLicences[i].info.currentLevel = 0;
                else
    ...
    public class TRCustomLicence {
        internal Color color = new(195f / 255f, 135f / 255f, 112f / 255f, 1);
        internal Dictionary<int, int> costs = new();

        // Descriptions must be set per-level
        internal Dictionary<int, string> descriptions = new();
        internal Licence info;
        internal Sprite licenceIcon;
        internal int licenceIndex;
        internal int maxLevel = 3;
        internal Dictionary<string, int> prereqsCustom = new();
        internal Dictionary<LicenceManager.LicenceTypes, int> prereqsVanilla = new();
        internal Dictionary<int, Dictionary<CharLevelManager.SkillTypes, int>> skillRequirements = new();
