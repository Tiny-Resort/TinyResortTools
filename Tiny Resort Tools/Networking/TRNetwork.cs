...
public class TRNetwork : NetworkBehaviour {
    ...
    public void CmdSendMessageToHost(string message) {
        ...
        writer.WriteUInt(NetworkMapSharer.share.localChar.netId);
        SendCommandInternal(typeof(TRNetwork), "CmdSendMessageToHost", writer, 0, false);
        NetworkWriterPool.Recycle(writer);
    }

    protected static void InvokeUserCode_CmdSendMessageToHost(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection) =>
    ...
    internal void UserCode_TargetSendMessageToClient(string message) => TRTools.LogError($"Sending message to Client: {message}");
    ...
    static TRNetwork() {
        ...
        RemoteCallHelper.RegisterCommandDelegate(typeof(TRNetwork), "CmdSendMessageToHost", new CmdDelegate(InvokeUserCode_CmdSendMessageToHost), false);
        RemoteCallHelper.RegisterCommandDelegate(typeof(TRNetwork), "TargetSendMessageToClient", new CmdDelegate(InvokeUserCode_TargetSendMessageToClient), false);
