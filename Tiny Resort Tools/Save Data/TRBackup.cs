...
internal class TRBackup {
    ...
    private static List<FileInfo> currentBackups = new();
    ...
    private static void getOrCreateSavePaths() {
        ...
        if (CustomSavePath.Value.IsNullOrWhiteSpace())
            saveDestinationSlot = Path.Combine(SaveDestination, "Slot" + saveSlot.ToString());
        else
            saveDestinationSlot = Path.Combine(CustomSavePath.Value, "Slot" + saveSlot);
    ...
    internal static void CreateInitialBackup() {
        ...
        foreach (var file in currentGlobalSaves) allCurrentSaves.Add(Path.GetFileName(file));
