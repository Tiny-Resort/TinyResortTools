...
public class TRAssets {
    ...
    internal static bool checkFileSignature(string file) {
        ...
        using (var binary_reader = new BinaryReader(File.Open(file, FileMode.Open))) {
            var data = binary_reader.ReadBytes(0x10);
            var data_as_hex = BitConverter.ToString(data).Trim();
    ...
    internal static AssetBundle LoadAssetBundleFromDLL(string name) {
        ...
        var CurrentAssemblyResourcePaths = CurrentAssembly.GetManifestResourceNames().ToList();
        var TargetResourcePath = CurrentAssemblyResourcePaths.Find(CurrentResource => CurrentResource.Contains(name));
    ...
    internal static Sprite LoadTextureFromAssetBundle(AssetBundle myAssetBundle, string textureName, Vector2 pivot) {
        var tex = myAssetBundle.LoadAsset<Texture2D>(textureName);
        var mySprite = Sprite.Create(tex, new Rect(0.0f, 0.0f, tex.width, tex.height), pivot, 100.0f);
        return mySprite;
    }
    ...
    public static Sprite LoadSprite(string relativePath, Vector2 pivot) {
        var tex = LoadTexture(relativePath);
        var newSprite = Sprite.Create(tex, new Rect(0, 0, tex.width, tex.height), pivot, 100);
        return newSprite;
    }
