    ...
    internal class ItemChangerData : ItemSaveData {
        ...
        public static void Save(int itemID, CurrentChanger changer) {
            ...
            if (changer.houseX <= 0 && changer.houseY <= 0)
                WorldManager.Instance.onTileStatusMap[changer.xPos, changer.yPos] = -2;
            else
                HouseManager.manage.getHouseInfo(changer.houseX, changer.houseY)
                    .houseMapOnTileStatus[changer.xPos, changer.yPos] = -2;
            WorldManager.Instance.allChangers.Remove(changer);
        ...
        public TRCustomItem Load() {
            ...
            if (houseXPos <= 0 && houseYPos <= 0)
                WorldManager.Instance.onTileStatusMap[objectXPos, objectYPos] = customItem.inventoryItem.getItemId();
            else
            ...
            //WorldManager.Instance.allChangers.Add(restoreChanger);
            WorldManager.Instance.loadCountDownForTile(restoreChanger);
            return customItem;
        }
    }
}
