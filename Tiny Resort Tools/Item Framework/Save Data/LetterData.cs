...
internal class LetterData : ItemSaveData {
    ...
    public static List<LetterData> all = new();
    public static List<LetterData> lostAndFound = new();
    ...
    public static void Save(Letter toRemove, bool tomorrow) {
        ...
        if (tomorrow)
            MailManager.manage.tomorrowsLetters.Remove(toRemove);
        else
            MailManager.manage.mailInBox.Remove(toRemove);

    }

    public TRCustomItem Load() {
        if (!TRItems.customItems.TryGetValue(customItemID, out var customItem)) return null;
        var letter = new Letter(sentById, myType, itemAttached, stackOfItemAttached);
