...
namespace TinyResort {
    ...
    internal class ObjectTopData : ItemSaveData {
        ...
        public static void Save(int tileObjectID, int ObjectXPos, int ObjectYPos, int rotation, int HouseXPos, int HouseYPos, int status, int onTopPos) {
            ...
            all.Add(new ObjectTopData {
                customItemID = TRItems.customTileObjectByID[tileObjectID].customItemID, rotation = rotation, onTopPos = onTopPos, status = status,
            ...
            if (HouseXPos > -1 && HouseYPos > -1 && HouseManager.manage.getHouseInfo(HouseXPos, HouseYPos) != null) {
                ...
                if (displayPlayerHouseTiles && displayPlayerHouseTiles.tileObjectsInHouse[ObjectXPos, ObjectYPos]) 
                    displayPlayerHouseTiles.tileObjectsInHouse[ObjectXPos, ObjectYPos].checkOnTopInside(ObjectXPos, ObjectYPos, HouseManager.manage.getHouseInfo(HouseXPos, HouseYPos));
            }
