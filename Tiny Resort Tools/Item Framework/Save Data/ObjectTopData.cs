    ...
    internal class ObjectTopData : ItemSaveData {
        ...
        public static void Save(int tileObjectID, int ObjectXPos, int ObjectYPos, int rotation, int HouseXPos, int HouseYPos, int status, int onTopPos) {
            ...
            if (TRItems.customTileTypeByID.ContainsKey(tileObjectID)) {
                all.Add(new ObjectTopData { customItemID = TRItems.customTileObjectByID[tileObjectID].customItemID, rotation = rotation, onTopPos = onTopPos, 
                            status = status, objectXPos = ObjectXPos, objectYPos = ObjectYPos, houseXPos = HouseXPos, houseYPos = HouseYPos, mannequinID = -1 });
            }
            else if (TRItems.customItemsByItemID.ContainsKey(status)) {
                all.Add(new ObjectTopData { customItemID = TRItems.customItemsByItemID[status].customItemID, rotation = rotation, onTopPos = onTopPos, status = status,
                            objectXPos = ObjectXPos, objectYPos = ObjectYPos, houseXPos = HouseXPos, houseYPos = HouseYPos, mannequinID = tileObjectID });
            }
