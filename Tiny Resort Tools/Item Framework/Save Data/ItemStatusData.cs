...
namespace TinyResort {
    ...
    internal class ItemStatusData : ItemSaveData {
        ...
        public static List<ItemStatusData> all = new List<ItemStatusData>();
        public static List<ItemStatusData> lostAndFound = new List<ItemStatusData>();
        ...
        public static void Save(int itemID, int objectXPos, int objectYPos, int houseXPos, int houseYPos) {
            all.Add(new ItemStatusData { customItemID = TRItems.customItemsByItemID[itemID].customItemID, objectXPos = objectXPos, objectYPos = objectYPos, houseXPos = houseXPos, houseYPos = houseYPos});
            var houseDetails = houseXPos == -1 ? null : HouseManager.manage.getHouseInfo(houseXPos, houseYPos);

            if (houseDetails != null) houseDetails.houseMapOnTileStatus[objectXPos, objectYPos] = -1; 
            else WorldManager.manageWorld.onTileStatusMap[objectXPos, objectYPos] = -1; 
        }
