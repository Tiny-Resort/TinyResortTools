...
namespace TinyResort {
    ...
    internal class InvItemData : ItemSaveData {
        ...
        public static void Save(int slotNo, int stackSize) {
            all.Add(
                new InvItemData {
                    customItemID = TRItems.customItemsByItemID[Inventory.Instance.invSlots[slotNo].itemNo].customItemID,
                    slotNo = slotNo, stackSize = stackSize
                }
            );
            Inventory.Instance.invSlots[slotNo].updateSlotContentsAndRefresh(-1, 0);
        }

        public TRCustomItem Load() {
            if (!TRItems.customItems.TryGetValue(customItemID, out var customItem)) return null;
            Inventory.Instance.invSlots[slotNo]
                .updateSlotContentsAndRefresh(customItem.inventoryItem.getItemId(), stackSize);
