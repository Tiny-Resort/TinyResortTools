...
public class TRQuickItems {
    ...
    internal static void InitializeCustomItem(string path, GameObject obj, QuickItemInfo itemInfo) {
        ...
        if (newItem.inventoryItem.craftable is not null) newItem.inventoryItem.craftable.showInRecipeOverflow = true;
        ...
        if (TRItems.customItems.ContainsKey(newItem.customItemID)
         && itemInfo.type.ToLower().Replace(" ", "").Trim() == "path") {
            ...
            if (ColorUtility.TryParseHtmlString(itemInfo.color, out var color))
                newItem.tileTypes.tileColorOnMap = color;
        }
...
public class QuickItemInfo {
    ...
    public void DisplayInfo() {
        TRTools.LogError($"{itemName}");
        TRTools.LogError($"\tNexus ID: {nexusID}");
        TRTools.LogError($"\tUnique ID: {uniqueID}");
        TRTools.LogError($"\tFile Name: {fileName}");
        TRTools.LogError($"\tType: {type}");
        TRTools.LogError($"\tDescription: {description}");
        TRTools.LogError($"\tValue: {value}");
        TRTools.LogError($"\tColor: {color}");
        TRTools.LogError($"\tNormal Map: {normalMapFileName}");
        TRTools.LogError($"\tIcon Name: {iconFileName}");
    }
}
