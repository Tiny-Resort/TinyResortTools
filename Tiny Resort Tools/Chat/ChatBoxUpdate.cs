...
internal class Update {
    ...
    public static void Postfix(ChatBox __instance) {
        if (!NetworkMapSharer.Instance || !NetworkMapSharer.Instance.localChar || Inventory.Instance.menuOpen) return;
    ...
    public static void Prefix(ChatBox __instance) {
        ...
        var parameters = __instance.chatBox.text.Split(' ');
        ...
        var foundCommands = TRChat.GetMatchingCommands(trigger, parameters[1]);

        if (foundCommands.Count <= 0) {
            if (TRChat.predictedTriggers.Count > 0) {
                var message = "Did you mean to use the command: ";
                for (var i = 0; i < TRChat.predictedTriggers.Count; i++) {
            ...
            else { TRChat.SendError("\"/" + trigger + " " + parameters[1] + "\" is not a valid command."); }
            return;
        }
